...
<span style="color: purple;">I recommend breaking up all this text since all the bullets from the original page smushed together. It looks a bit messy </span>
```{r}
...
<span style="color: purple;">this looks like difference of sums and not proportion.. but I think u fixed this in person yesterday</span>
```{r}
# One tailed z test
mean(monkeys$weight)
mu <- 5.0
...
<span style="color: purple;">So for this first part of the homework, we are creating 1 function z.prop.test() that can run *both* a one-sample and two-sample test. Also I recommend going back through the question because you should include all the arguments listed, including "alternative = two.sided" and the "less" "greater" alternatives. Also your 2-sample proportion test has slightly different steps than the one sample... you need pstar as well as the warning message if the rule of thumb is not met!!... but i think we went over all this together in person </span>
...
<span style="color: purple;">again, could be a good idea to break up all this text!</span>
```{r}
library(ggplot2)
